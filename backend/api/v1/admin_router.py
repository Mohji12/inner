...
def admin_analytics(
    ...
    # Live presence (memory + recent last_seen_at). When a single coach filter is set, 0 or 1.
    if cid:
        m = db.query(Mentor).filter(Mentor.id == cid).first()
        online_mentors = (
            1
            if m and presence_service.is_online(cid, "mentor", last_seen_at=m.last_seen_at)
            else 0
        )
    else:
        online_mentors = presence_service.count_online("mentor")
        # Augment with DB-fresh heartbeats not yet seen by this worker.
        ...
        db_online = (
            db.query(Mentor.id)
            .filter(Mentor.last_seen_at.isnot(None), Mentor.last_seen_at >= cutoff, Mentor.status == "active")
            .count()
        )
        online_mentors = max(online_mentors, int(db_online))
