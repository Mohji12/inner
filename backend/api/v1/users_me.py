...
class WelcomePromoOut(BaseModel):
    eligible: bool
    code: str | None = None
...
@router.get("/me/welcome-promo", response_model=WelcomePromoOut)
def read_welcome_promo(db: DbSession, user: CurrentUser) -> WelcomePromoOut:
    data = welcome_promo_eligibility(db, user.id)
    return WelcomePromoOut(**data)
...
def user_presence_status(user: CurrentUser) -> UserPresenceStatusOut:
    return UserPresenceStatusOut(
        is_online=presence_service.is_online(user.id, "user", last_seen_at=user.last_seen_at)
    )
