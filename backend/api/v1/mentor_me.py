...
from services.mollie_service import MollieServiceError, resolve_frontend_return_origin, resolve_mollie_webhook_url
...
def mentor_set_manual_occupied(
    db: DbSession,
    me: CurrentMentor,
...
def mentor_create_onboarding_payment(
    ...
    redirect_url = f"{resolve_frontend_return_origin(request, claimed_origin=payload.return_origin)}/mentor?onboardingPaid=1"
...
def mentor_prepare_monthly_invoice_checkout(
    ...
    try:
        ...
        ensure_monthly_invoice_mollie_checkout(
            ...
            webhook_url=resolve_mollie_webhook_url(request),
            force_new=False,
            redirect_url=f"{origin}/mentor/invoices?invoicePaid=1",
        )
