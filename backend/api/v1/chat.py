...
def start_instant_session(
    ...
    try:
        origin = resolve_frontend_return_origin(request, claimed_origin=payload.return_origin)
        session, checkout_url, mollie_pid = start_session_checkout(
            ...
            redirect_url=f"{origin}/user/chat/{{session_id}}?checkout=return",
...
def extend_chat_session(
    ...
    try:
        origin = resolve_frontend_return_origin(request, claimed_origin=payload.return_origin)
        mode = (payload.communication_mode or "").strip().lower()
        qs = "extended=1"
        if mode in ("call", "video"):
            qs += f"&mode={mode}"
        redirect_url = f"{origin}/user/chat/{session_id}?{qs}"
...
def get_chat_session(session_id: str, db: DbSession, actor: ChatActorDep) -> ChatSessionOut:
    ...
    if actor.user:
        mentor_seen = db.query(Mentor.last_seen_at).filter(Mentor.id == session.mentor_id).first()
        # In-room: do not mask coach as offline just because they are busy in this (or another) session.
        partner_online = presence_service.is_online(
            session.mentor_id,
            "mentor",
            last_seen_at=mentor_seen[0] if mentor_seen else None,
        )
    else:
        user_seen = db.query(User.last_seen_at).filter(User.id == session.user_id).first()
        partner_online = presence_service.is_online(
            session.user_id,
            "user",
            last_seen_at=user_seen[0] if user_seen else None,
        )
...
def list_my_chat_sessions(db: DbSession, actor: ChatActorDep) -> ChatInboxOut:
    ...
    for s in sessions:
        ...
        if actor.user:
            partner = (
                db.query(Mentor.full_name, Mentor.profile_image, Mentor.last_seen_at)
                .filter(Mentor.id == s.mentor_id)
                .first()
            )
            partner_online = presence_service.is_online(
                s.mentor_id,
                "mentor",
                last_seen_at=partner[2] if partner else None,
            ) and not mentor_chat_busy(db, s.mentor_id)
        else:
            partner = (
                db.query(User.full_name, User.profile_image, User.last_seen_at)
