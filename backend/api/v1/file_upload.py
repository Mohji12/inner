...
UPLOAD_DIR = "uploads"
MAX_IMAGE_BYTES = 2 * 1024 * 1024  # cropped / public `file`
MAX_ORIGINAL_BYTES = 12 * 1024 * 1024  # phone camera `original`
...
async def _read_image_upload(file: UploadFile, *, max_bytes: int = MAX_IMAGE_BYTES) -> bytes:
    ...
    if len(contents) > max_bytes:
        ...
        raise HTTPException(
            ...
            f"Image size should be less than {limit_mb} MB.",
        )
...
async def _read_optional_image_upload(file: UploadFile | None) -> bytes | None:
    ...
    if not filename:
        return None
    return await _read_image_upload(file, max_bytes=MAX_ORIGINAL_BYTES)
