...
def create_and_send_otp(
    ...
    *,
    ...
    try:
        sent = send_plain_email(to_email=email_l, subject=subject, body=body)
    ...
    if not sent:
        if (settings.environment or "").lower() == "development":
            logger.warning(
                "Development fallback: SMTP not configured; OTP for %s is %s",
                email_l,
                code,
            )
            return code
        raise RuntimeError(f"OTP email was not accepted by SMTP for {email_l}")
    return code
