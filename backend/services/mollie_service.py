...
from core.chat_states import (
    CHAT_PURCHASE_FAILED,
    CHAT_PURCHASE_SUCCEEDED,
    CHAT_SESSION_ACTIVE,
    CHAT_SESSION_ENDED,
    CHAT_SESSION_PAUSED,
)
...
def _apply_chat_purchase_paid(db: Session, purchase: ChatPurchase) -> None:
    ...
    try:
        ...
        mins = int(purchase.minutes)
        create_notification(
            db,
            type="chat_extended",
            title="Client added session time",
            body=f"Your client purchased {mins} more minute{'s' if mins != 1 else ''}. The conversation is active again.",
            link=f"/mentor/chat/{session.id}",
            ...
            if already_paid_elsewhere:
                ...
                if promo_code:
                    apply_promo_code(
                        db,
                        promo_code,
                        user_id=booking_payment.user_id,
                        booking_id=booking_payment.booking_id,
                    )
        elif status_str in ("failed", "canceled", "expired"):
            booking_payment.status = "failed"
            booking = db.query(Booking).filter(Booking.id == booking_payment.booking_id).first()
            if booking and booking.payment_status != PAYMENT_PAID:
                from core.booking_states import PAYMENT_FAILED
