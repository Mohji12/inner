...
def ensure_monthly_invoice_mollie_checkout(
    ...
    *,
    ...
    resolved_redirect = (redirect_url or f"{settings.mollie_redirect_base_url.rstrip('/')}/mentor/invoices").rstrip(
        "/"
    )
    if "?" not in resolved_redirect:
        resolved_redirect = f"{resolved_redirect}?invoicePaid=1"
