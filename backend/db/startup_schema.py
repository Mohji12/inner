...
def ensure_promo_code_allowed_duration_column() -> None:
    _safe_add_column("ALTER TABLE promo_codes ADD COLUMN allowed_duration_minutes INT NULL")
...
def ensure_promo_code_redemptions_table() -> None:
    ddl = """
    CREATE TABLE IF NOT EXISTS promo_code_redemptions (
        ...
        UNIQUE KEY uq_promo_redemption_user_promo (user_id, promo_code_id),
        KEY ix_promo_code_redemptions_user_id (user_id),
        KEY ix_promo_code_redemptions_promo_code_id (promo_code_id),
        KEY ix_promo_code_redemptions_booking_id (booking_id),
        CONSTRAINT fk_promo_redemptions_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
        CONSTRAINT fk_promo_redemptions_promo FOREIGN KEY (promo_code_id) REFERENCES promo_codes(id) ON DELETE CASCADE,
        CONSTRAINT fk_promo_redemptions_booking FOREIGN KEY (booking_id) REFERENCES bookings(id) ON DELETE SET NULL
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
    """
    with engine.begin() as conn:
        conn.execute(text(ddl))
...
def ensure_user_welcome_promo_code() -> None:
    """Create the configured new-user booking waiver promo if it does not exist."""
    from datetime import datetime, timezone

    from core.config import settings
    from core.security import new_uuid

    code = (settings.user_welcome_promo_seed_code or "").strip().upper()
    if not code:
        return
    ...
    with engine.begin() as conn:
        row = conn.execute(
            text("SELECT id FROM promo_codes WHERE code = :code LIMIT 1"),
            {"code": code},
        ).first()
        if row:
            conn.execute(
                text(
                    ...
                    SET scope = 'all',
                        ...
                        first_time_only = 1,
                        allowed_duration_minutes = :duration,
                        is_active = 1
                    WHERE code = :code
                    """
                ),
        ...
        conn.execute(
            text(
                """
                INSERT INTO promo_codes (
                    id, code, discount_type, discount_value, scope, is_active,
                    current_uses, first_time_only, allowed_duration_minutes, created_at
                ) VALUES (
                    :id, :code, 'percentage', 100.00, 'all', 1, 0, 1, :duration, :created_at
                )
                """
            ),
            {
...
def ensure_mentor_manual_occupied_column() -> None:
...
def ensure_email_otp_role_width() -> None:
    """password_reset:mentor is 21 chars; migration 003 used VARCHAR(16)."""
    try:
        _execute_ddl(
            "ALTER TABLE email_otp_codes MODIFY COLUMN role VARCHAR(32) NOT NULL "
            "COMMENT 'user|mentor|password_reset:user|password_reset:mentor'"
        )
    except DBAPIError as e:
        # Table may not exist yet on brand-new DBs that have not run 003.
        msg = str(e).lower()
