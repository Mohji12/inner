...
export function ChatPanel({ sessionId, session }: Props) {
  ...
  const timeExpired = sessionTimeExpired(session);
  const joinDead = sessionJoinWindowExpired(session);
  ...
  const wsBarStatus =
    wsStatus === "reconnecting" || wsStatus === "disconnected" || wsStatus === "connected"
      ? wsStatus
      : "connected"; // hide bar while initial connecting / auth settling

  return (
    <Card className="border-border/60 shadow-xl shadow-black/5 overflow-hidden">
      <ConnectionStatusBar status={wsBarStatus} />
