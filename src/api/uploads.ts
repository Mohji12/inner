...
import { appendNamedFile, compressImageForUpload } from "@/lib/imageUpload";
...
async function appendPhotoForm(params: {
  cropped: File;
  original?: File | null;
  ...
  if (params.original) {
    const compressed = await compressImageForUpload(params.original);
    appendNamedFile(body, "original", compressed, "original.jpg");
  }
...
export async function uploadMentorPhoto(params: {
  ...
  return apiFetch<MentorPhotoUploadResult>(path, {
    method: "POST",
    body: await appendPhotoForm(params),
  });
}
...
export async function uploadRegistrationMentorAvatar(params: {
  ...
  const body = await appendPhotoForm({
    cropped: params.file,
