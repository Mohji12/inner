...
const MentorRegisterPage = () => {
  ...
  const onTabChange = (value: string) => {
    ...
    setTab(value);
    navigate(`/mentor/register?tab=${value}`, { replace: true });
  };
  ...
  const formatError = (err: unknown, fallback: string) => {
    ...
    if (/connection problem|load failed|failed to fetch/i.test(msg)) {
      return m.errConnection ?? msg;
    }
    return msg;
  };
  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const msg = formatError(err, m.errFailed);
      if (phase !== "verify") {
        setError(msg);
      }
      toast.error(msg);
  ...
  const onVerifyOtp = async () => {
    ...
    try {
      ...
      const msg = formatError(err, m.errVerify);
      setError(msg);
      toast.error(msg);
    } finally {
      setVerifying(false);
  ...
  const onResendOtp = async () => {
    if (!verifyCtx || submitting) return;
    try {
            ...
            {phase === "verify" ? (
              <div className="space-y-6">
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  <Button type="button" variant="ghost" onClick={() => void onResendOtp()} disabled={verifying || submitting}>
                    {m.resendCode}
                  </Button>
                  <Button
                    type="button"
            ...
            <form lang={htmlLang} onSubmit={(e) => void onSubmit(e)} className="space-y-6">
              ...
              {tabIndex === TAB_ORDER.length - 1 ? (
                <div className="flex flex-wrap justify-end gap-3">
                  ...
                  <Button type="submit" className="gradient-cta text-white" disabled={submitting}>
                    {m.submit}
                  </Button>
                </div>
              ) : null}
