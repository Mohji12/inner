  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        if (cancelled) return;
        clearPendingMolliePaymentId();
        syncStatus = String(out.status || "").toLowerCase();
      } catch {
        /* webhook may arrive later */
      } finally {
        if (!cancelled) {
          await loadBookingDetails();
          if (["failed", "canceled", "cancelled", "expired"].includes(syncStatus)) {
            setPageState((prev) => (prev === "paid" ? prev : "failed"));
          }
        }
      }
    })();
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex flex-wrap gap-3 mt-2 justify-center">
              ...
              <Button asChild className="gradient-cta text-white">
                <Link to={bookingId ? `/user/appointments?bookingId=${encodeURIComponent(bookingId)}` : "/user/appointments"}>
                  {b.goAppointments}
                </Link>
              </Button>
              {pageState === "failed" && booking && mentor ? (
                <Button asChild variant="default">
                  <Link to={`/payment/${mentor.id}?bookingId=${booking.id}`}>Retry payment</Link>
                </Button>
              ) : null}
