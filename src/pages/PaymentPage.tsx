...
const PaymentPage = () => {
  ...
  const welcomePromoAppliedRef = useRef(false);
  ...
  const welcomePromoQuery = useQuery({
    ...
    enabled: Boolean(booking?.duration === 5),
  });
  ...
  useEffect(() => {
    ...
    if (!wp?.eligible || !wp.code || !mentor || !booking || booking.duration !== wp.duration_minutes) return;
    if (welcomePromoAppliedRef.current || promoCode) return;

    welcomePromoAppliedRef.current = true;
    const code = wp.code;
    ...
    const amount =
      checkoutPreviewQuery.data?.session_amount_eur ??
      (pricing ? sessionPackageEur(mentor, pricing, booking.duration) : 0);

    void validatePromoCode(code, amount, mentor.id, booking.duration)
      .then((res) => {
        ...
        if (res.is_valid) {
          ...
          welcomePromoAppliedRef.current = false;
  ...
  const handleApplyPromo = async () => {
    ...
    setPromoError("");
    try {
      const res = await validatePromoCode(promoCodeInput.trim(), baseAmount, mentor.id, booking.duration);
      setTransactionFee(res.transaction_fee);
      if (res.is_valid) {
              ...
              <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                ...
                {totalDue > 0 && !canPayFromWallet ? (
                  <Button type="button" variant="outline" className="w-full sm:w-auto" asChild>
                    <Link
                      to={`/user/wallet?returnTo=${encodeURIComponent(`/payment/${mentor.id}?bookingId=${booking.id}`)}`}
                    >
                      {p.addMoney}
                    </Link>
                  </Button>
