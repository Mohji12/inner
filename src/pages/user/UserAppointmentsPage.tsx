                ...
                <CardContent className="space-y-3 p-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {s.status === "ended" && !unpaid && !timeUp && !joinDead ? (
                      ...
                      <Button size="sm" variant={unpaid || timeUp || joinDead ? "default" : "outline"} className={unpaid || timeUp || joinDead ? "gradient-cta text-white" : undefined} asChild>
                        <Link to={`/user/chat/${s.id}`}>{openLabel}</Link>
                      </Button>
                    )}
                  </div>
                </CardContent>
                  ...
                  {b.status === "pending_payment" && b.payment_status === "unpaid" ? (
                    canWalletPay ? (
                      ...
                      <>
                        ...
                        >
                          <Link
                            to={`/user/wallet?returnTo=${encodeURIComponent(`/user/appointments?bookingId=${b.id}`)}`}
                          >
                            {ap.addMoney}
                          </Link>
                        </Button>
