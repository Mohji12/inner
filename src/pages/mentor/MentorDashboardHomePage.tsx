...
export default function MentorDashboardHomePage() {
  ...
  useEffect(() => {
    ...
    const flag = searchParams.get("onboardingPaid");
    ...
    void (async () => {
      try {
        if (pending) {
          ...
          if (st === "paid") toast.success("Onboarding payment confirmed");
          else if (["failed", "canceled", "cancelled", "expired"].includes(st)) toast.error("Onboarding payment was not completed");
          else toast.info("Onboarding payment is still processing");
        }
        await Promise.all([
          queryClient.invalidateQueries({ queryKey: ["mentor", "onboarding-payments"] }),
          queryClient.invalidateQueries({ queryKey: ["mentor", "onboarding-status"] }),
          queryClient.invalidateQueries({ queryKey: ["mentor", "monthly-invoices"] }),
        ]);
      } catch {
        if (!cancelled) toast.info("Onboarding payment is still processing");
      } finally {
        if (!cancelled && flag) {
          const next = new URLSearchParams(searchParams);
          next.delete("onboardingPaid");
  ...
  const payInstallmentMut = useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (data.payment_id) stashPendingMolliePaymentId(data.payment_id);
